        ...
        <div v-else class="max-w-4xl px-10 my-4 py-6 bg-custom-white rounded-lg shadow-md">
            ...
            <div class="text-xl font-light py-8">
                <template v-if="goodResult">
                    <p>Bravo <span class="font-bold">{{ userName }}</span> <span class="text-2xl">&#127881;</span> Tu es une
                        personne cultivée ! Sois fier de toi et continue à
                        t'intéresser au monde qui t'entoure <span class="text-2xl">&#127757;</span></p>
                    <VictoryAnimation />
                </template>
                <template v-else>
                    <p>Ton score n'est pas fou <span class="font-bold">{{ userName }}</span> <span
